...
*/
const spanny = document.querySelector('#createdBy');
spanny.innerHTML += '<p>Alex Ager.</p>';
...
/*
  STEP 4: This function, buildToDo, will have two parameters.  The first is an object of class ToDo and 
          ...
          Inside this function it should:
            ...
            4.) Using the toDo item passed in, set the 'toDoText' innerHTML to the value of the 
            'description' property on the toDo object.
            5.) Set the id of 'toDoText' to the value of the index argument.
            6.) Using an if statement, check to see if the 'complete' property
             on the object passed as the first argument is set to true. If it is, give
             'toDoText' a CSS class of 'completeText'. If it is not, do not give it a class.
...
/* 
  ...
  STEP 5: This function will build and return an array of toDo elements. It will take an 
          array of objects of the ToDo class as it's only argument.
          
          Using the map method on the array passed in, use the 'buildToDo' 
          function you wrote above as the callback passed to map. 
...
/* 
  STEP 7: This function, 'addToDo' will add a new ToDo to the 'toDoItems' array.
      NOTE: We have not learned about input HTML elements yet,
      so we will give you a little more code to go on here.
         
          'toDoInput' is an text input element. All text input 
          elements have a property called 'value', this value will 
          be whatever is typed into the text box on the page.
          1.) Using the value property on 'toDoInput', create a new ToDo object using the ToDo class
          and pass the value as the description.
              2.) add the object from (1) into the toDoItems array.

          3.) Set the value of toDoInput to an empty string (this 
          will clear the text in the box allowing the user to enter another item).
...
/* 
  STEP 8: In this step we will fire addToDo everytime the 'ADD' button is clicked.
          1.) Select the element with the id 'addButton'
              2.) Add a 'click' event listener to this element, 
              passing it the addToDo function as a callback
*/
...
/* 
  STEP 9: Finally in this step we will define the function to run when we want to 
  complete a toDo, and add that function to the click event listener on the toDo element
          
          Note: We have not covered the argument every event listener receives, 
          the 'event' object. There is a lot of data in this object, 
          including event type, which element called it, what the values
          of that element are, etc. In this exercise we will use it to 
          find the index of the item that called it. We have given you 
          that code, study it to make sure you understand what is happening.
