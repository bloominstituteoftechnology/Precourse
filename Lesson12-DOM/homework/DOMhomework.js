...
newP.innerHTML = 'Casey Baker';

const parentDiv = document.querySelector('#createdBy');
...
function buildToDos(toDos) {
  const newToDo = toDos.map(buildToDo);
  return newToDo;
  document.getElementById('toDoText').addEventListener("click", completeToDo(event));
}
...
function displayToDos() {
    ...
    buildToDos(toDoItems).forEach(function(elem) {
        toDoContainer.innerHTML += elem;
    });
...
document.getElementById('addButton').addEventListener("click", addToDo(addButton));
