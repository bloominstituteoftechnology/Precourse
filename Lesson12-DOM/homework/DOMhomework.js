...
function buildToDos(toDos) {
  // code here
  const result = toDos.map(buildToDo());
  return result; 
}
...
function displayToDos() {
  ...
  const ToDo = buildToDos(toDoItems);
  for(let i = 0; i < ToDo.length; i++) {
    ToDo[i].appendChild(toDoContainer);
  }
...
function addToDo() {
  ...
  let toDo2 = new ToDo(newToDo.value);
  newToDo = " ";
  displayToDos();
