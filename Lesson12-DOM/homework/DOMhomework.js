...
createdBy.innerHTML += (' Hilal Aissani');
/* 
...
function buildToDo(todo, index) {
  // code here
let toDoShell = document.creatElement('div');
toDoShell.className  = 'toDoShell';
let toDoText = document.creatElement('span');
...
function buildToDos(toDos) {
  // code here
    let mapped = toDos.map(buildToDo);
    return mapped; 
...
function displayToDos() {
  ...
  let displaytodos = buildToDos(toDoItems);
  for ( i = 0 ; i < displaytodos.length;i++){
    toDoContainer.appendChild(displaytodos[i]);
  };
}
