...
function buildTodos() {
  ...
  */
  let doArray= todoObject.map(buildTodo);
  return doArray;
}
...
function displayTodos() {
  ...
  */
  let containd = document.querySelector('#todoContainer');
  containd.innerHTML = '';
  let itemArray = buildTodos(toDoItems);
  for (let i = 0; i < itemArray.length; i++) {
    containd.appendChild(itemArray[i]);
  }
...
function addTodo() {
  ...
  */
  ...
  //UNCOMMENT THE NEXT LINE
   ...
   let todoObj = new Todo(newTodo.value);
   newTodo.value = '';
   displayTodos();
}
