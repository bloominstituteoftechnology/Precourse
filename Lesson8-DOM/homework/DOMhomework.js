...
created.innerHTML = 'This app was created by: Amanda Moc';
...
function buildTodo(object, index) {
  ...
  if (object.complete === true) {
    todo.Text.className = 'completeText';
  };
  ...
  */
  function displayTodos() {
    let DispFunc = document.querySelector('#todoContainer');
    DispFunc.innerHTML = '';
    let DispArr = buildTodos(toDoItems);
      for (let i = 0; i < DispArr.length; i++) {
        DispFunc.appendChild(DispArr[i]);
      }
}
  ...
  */
  ...
  function addTodo() {
    let newTodo = document.querySelector('#todoInput');
    let NewTask = new Todo(newTodo.value + '');
