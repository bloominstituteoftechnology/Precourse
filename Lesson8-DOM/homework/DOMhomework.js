...
created.innerHTML = created.innerHTML + " Aaron Faulkner";
...
const toDoItems = [new Todo('Description')];
...
function displayTodos() {
  ...
  */
  ...
  todoList.innerHTML = '';
  const listArr = buildTodos(toDoItems);

  for (let i = 0; i < listArr.length; i++){
    todoList.appendChild(listArr[i]);
  }
...
function addTodo() {
  ...
  */
  ...
  let anotherTodo = new Todo(newTodo.value);
  toDoItems.push(anotherTodo);
  Todo.value = '';
  displayTodos();
