...
created.innerHTML = created.innerHTML + ' Elizabeth Ashley Flack';
...
function buildTodo(object, index) {
  ...
  todoShell.className = 'todoShell'; // accidentally had this has double quotations
  let todoText = document.createElement('span');  // accidentally had this has double quotations
 ...
 //In the buildTodo function add a 'click' event listener to the 'todoText' element, and pass this function as the callback.
...
function buildTodos(arrayOfTodos) {
    let newArray = arrayOfTodos.map(buildTodo);
    return newArray;
  /*
...
function displayTodos() {
  ...
  for (let i = 0; i < toDoItemsArray.length; i++) {
      todoContainer.appendChild(toDoItemsArray[i]);
    }
  /*
...
function addTodo() {
  ...
  */
  ...
  let newTodo = document.querySelector('#todoInput');  // input object
  let newTodoObj = new Todo(newTodo.value);  // Todo('dinah')
  //newTodoObj.description = new.Todo.value;
  toDoItems.push(newTodoObj);  //toDoItems.push();
  newTodo.value = '';
  displayTodos();
}
/*
