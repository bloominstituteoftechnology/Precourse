...
/* STEP 3: Add a completeTodo method to the prototype of Todo. It will not take any arguemnts. Inside the function set the Todo's complete to true*/
Todo.prototype.completeTodo = function() {
  this.complete = true;
};

/* STEP 4: initiate an array called 'toDoItems'. In this array you should have one new object of the class Todo. */
  let homework = new Todo('Do this homework and my college homework as well.');
  let toDoItems = [homework,];
...
/*
 STEP 5: This function, buildTodo, will take an object of class Todo as it's first argument and 
         ...
         Inside this function should:
           ...
           8.) return todoShell
*/
function buildTodo(taskObj, index) {
  const todoShell = document.createElement('div');
  todoShell.className = 'todoShell';

  const todoText = document.createElement('span');
  todoText.innerHTML = taskObj.description;
  todoText.id = index;

  if (taskObj.complete === true) todoText.className = 'completeText';
...
*/
function buildTodos(tasksArr) {
  return tasksArr.map(buildTodo);
}
...
function displayTodos() {
  ...
  container.innerHTML = '';
  let tasksArr = buildTodos(toDoItems);
    for (let i = 0; i < tasksArr.length; i++) {
      container.appendChild(tasksArr[i]);
    }
}
...
function addTodo() {
  ...
  let inputTodo = new Todo(newTodo.value);
