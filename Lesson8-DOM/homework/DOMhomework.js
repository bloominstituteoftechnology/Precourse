...
created.innerHTML += ' Abraham Bueno'
...
function buildTodo(Todo,numerical) {
  let todoShell = document.createElement('div') ;
  todoShell.className = 'todoShell';
  let todoText = document.createElement('span');
  todoText.innerHTML = 'description';
  todoText.id = 'numerical';
  if ('complete' === Todo) {
    todoText.className = 'commpleteText';
  } else {
    todoText.className = '';
  }
  todoShell.appendChild(todoText);
  todoText.addEventListener('mouseenter', function() {
    buildTodo();
  }
...
function buildTodos(arrayOfTodos) {
  let nArr = arrayOfTodos.map(buildTodo);
   return nArr;
  // for (let i = 0; bTarr > i; i++) {
  //
  // }
  /*
...
function displayTodos(arrayOfTodos) {
  let darr = document.getElementById('todoContainer');
  darr.innerHTML[0] = '';
  let btarr = buildTodos(toDoItems);
  for (let i = 0; btarr.length > i; i++) {
    darr[i] = btarr[i];
  }
  return darr;
  /*
