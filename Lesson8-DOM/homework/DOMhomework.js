...
created.innerHTML = created.innerHTML + " " + "Adam Anderson";
// OK


// STEP 2
// 
// Create a class constructor called 'Todo' this function should take one argument, the description of the todo.
...
//
// Following nabsed's example, had to change Test "desc" to "description" to get this to pass.
...
//         
//         Following nabsed's example, had to change Test "completeToDo" to "completeTodo"
...
function buildTodos(arrTodo) {
  let mapArray = arrTodo.map(buildTodo);
  return mapArray;

// STEP 6: This function will build and return an array of todo element. It will take an array of objects of the Todo class
...
function displayTodos() {
  ...
  for (let i=1; i < itemsArray.length; i++) {
    containerTodo.appendChild(itemsArray[i]);
  }
...
//      2.) Set the innerHTML of the element from (1) to and empty string. (This will let us refresh the elements, and
//          display the new todos)
...
//      5.) at the very end of this file, the line before the comment "DO NOT CHANGE ANY CODE BELOW THIS LINE", call
//          this function.
//
...
//          'newTodo' is an text input element. All text input elements have a property called 'value', this value will
...
//      3.) Set the value of newTodo to an empty string (this will clear the text in the box allowing the user to enter
//          another item).
...
// UNCOMMENT THE NEXT LINE
  ...
  newTodo.value = '';
  newTodo.focus(); // had no idea about this; thanks nabsed!!
  displayTodos();
...
//  STEP 10: Finally in this step we will define the function to run when we want to compelte a todo, and add that function
//           to the click event listener on the todo element
//     Note: We have not covered the argument every event listener receives, the 'event' object. There is a lot of data in
//          this object, including event type, which element called it, what the values of that element are, etc. In this
//          exercise we will use it to find the index of the item that called it. We have given you that code, study it to
...
//      3.) In the buildTodo function add a 'click' event listener to the 'todoText' element, and pass this function as the
//          callback.
// UNCOMMENT THE NEXT LINE
