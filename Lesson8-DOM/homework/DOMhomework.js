...
*/
  function buildTodo(todoObj, index) {
    ...
    todoText.innerHTML = 'description'; //todoObj.description;
    todoText.setAttribute('id', index);
    
    todoText.onclick = completeTodo;

    if (todoObj.complete) todoText.className = 'completeText';
...
let mappedArr = objArr.map(buildTodo);
return mappedArr;
}
...
function displayTodos() {
  /* 
    STEP 7: Now that we can build an array of todo elements, we want to make these elements appear on the screen, 
            ...
            2.) Set the innerHTML of the element from (1) to an empty string. (This will let us refresh the elements, and display the new todos)
...
//UNCOMMENT THE NEXT LINE
let button = document.querySelector('#addButton').addEventListener('click', addTodo);
