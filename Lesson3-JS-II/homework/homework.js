...
function greeting(language) {
  ...
  const greet = ['Guten Tag','Hello','Hola'];
...
function averageTestScore(testScores) {
  ...
  // return the average
  let d = testScores.length;
  let tot = 0;
  for (const i=0;i<testScores.length;i++)
  {
  	tot += testScores[i];
  }
  let avg = tot / d;
...
function largestNumber(numbers) {
  ...
  let big = numbers[0];
  for(const i = 1;i<numbers.length;i++)
  {
