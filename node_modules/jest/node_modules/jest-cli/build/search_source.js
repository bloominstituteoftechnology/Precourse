'use strict';Object.defineProperty(exports, "__esModule", { value: true });var _fs;
...
function _load_micromatch() {return _micromatch = _interopRequireDefault(require('micromatch'));}var _jestResolveDependencies;
function _load_jestResolveDependencies() {return _jestResolveDependencies = _interopRequireDefault(require('jest-resolve-dependencies'));}var _test_path_pattern_to_regexp;
function _load_test_path_pattern_to_regexp() {return _test_path_pattern_to_regexp = _interopRequireDefault(require('./test_path_pattern_to_regexp'));}var _jestRegexUtil;
function _load_jestRegexUtil() {return _jestRegexUtil = require('jest-regex-util');}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}function _asyncToGenerator(fn) {return function () {var gen = fn.apply(this, arguments);return new Promise(function (resolve, reject) {function step(key, arg) {try {var info = gen[key](arg);var value = info.value;} catch (error) {reject(error);return;}if (info.done) {resolve(value);} else {return Promise.resolve(value).then(function (value) {step("next", value);}, function (err) {step("throw", err);});}}return step("next");});};} /**
...
const pathToRegex = p => (0, (_jestRegexUtil || _load_jestRegexUtil()).replacePathSepForRegex)(p);
...
class SearchSource {
  ...
  constructor(context) {const
    config = context.config;
    this._context = context;
    this._rootPattern = new RegExp(
    config.roots.map(dir => (0, (_jestRegexUtil || _load_jestRegexUtil()).escapePathForRegex)(dir)).join('|'));
  ...
  findRelatedTests(allPaths) {
    const dependencyResolver = new (_jestResolveDependencies || _load_jestResolveDependencies()).default(
  ...
  changedFilesPromise)
  {var _this = this;return _asyncToGenerator(function* () {var _ref =
      yield changedFilesPromise;const repos = _ref.repos,changedFiles = _ref.changedFiles;

      // no SCM (git/hg/...) is found in any of the roots.
      const noSCM = Object.keys(repos).every(function (scm) {return repos[scm].size === 0;});
      return noSCM ?
      { noSCM: true, tests: [] } :
      _this.findRelatedTests(changedFiles);})();
  }
  ...
  {var _this2 = this;return _asyncToGenerator(function* () {
      ...
      if (globalConfig.onlyChanged) {
        ...
        const allFiles = new Set(_this2._context.hasteFS.getAllFiles());
        const validTestPaths =
        paths &&
        paths.filter(function (name) {
          const fullName = (_path || _load_path()).default.resolve(name);

          try {
            if (!(_fs || _load_fs()).default.lstatSync(fullName).isFile()) {
          ...
          if (!_this2._testPathCases.testPathIgnorePatterns(fullName)) {
            return false;
          }
        ...
        if (validTestPaths && validTestPaths.length) {
          return Promise.resolve({ tests: toTests(_this2._context, validTestPaths) });
        } else if (globalConfig.testPathPattern != null) {
          return Promise.resolve(
          _this2.findMatchingTests(globalConfig.testPathPattern));
