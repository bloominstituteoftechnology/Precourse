...
function multiplyArguments() {
  ...
  return !arguments.length ? 0 : arguments.length === 1 ? arguments[0] 
  : Array.from(arguments).reduce((next, current) => current * next);
}
...
function sumArray(numbers, cb) {
  ...
  return cb(numbers.reduce((next, current) => next + current));
}
...
function createUserClass() {
  ...
  return function User({username, name, email, password}) {
...
function addReverseString() {
  ...
  String.prototype.reverse = () => this.reverse();
}
