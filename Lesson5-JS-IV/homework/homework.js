...
function multiplyArguments() {
  ...
  // if one argument is passed in just return it
    ...
    return (args.reduce(function (acc, item){return acc * item;}));
}
...
function forEach(arr, cb) {
  ...
  // hint: you will be invoking cb multiple times (once for each value in the array)
    arr.forEach(function (item) {cb(item);});
}
...
function map(arr, cb) {
  ...
  // the new array should be the same length as the array argument
    return (arr.map(function (item) {return cb(item);}));
}
  ...
  // you will need to use 'this' inside of reverse
    function reverse(reversedStr){
        ...
        for (let i = this.length - 1; i >= 0; i--) {
            reversedStr = reversedStr + this[i];
        }
        return reversedStr;
    }
...
function cacheFunction(cb) {
  ...
  // and return 25 directly and will not invoke cb again
    var resultsSoFar = {};
    function helperF (arg) {
        if (arg in resultsSoFar) {
            return resultsSoFar[arg];
    }
        else {
            let ans = cb(arg);
            resultsSoFar[arg] = ans;
            return ans;
        }
    }
    return helperF;
