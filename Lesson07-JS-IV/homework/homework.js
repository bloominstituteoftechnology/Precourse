...
function hasEmail(user) {
  ...
  else if (user.hasOwnProperty('email') === false) {return false;}
  else {return true;}
}
...
function sumUserPostLikes(user) {
  ...
  for (let i = 0; i < user['posts'].length; i++){
    preSumArray.push(user['posts'][i].likes);
  }
  return preSumArray.reduce((x, y) => x + y);
}
...
function addCalculateDiscountPriceMethod(storeItem) {
  ...
  //storeItem.addCalculateDiscountPrice 
